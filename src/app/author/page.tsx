...
export async function generateMetadata({ searchParams }: AuthorPageProps): Promise<Metadata> {
  ...
  try {
    const data: AuthorDetailResponse = await fetchAuthorDetail(id);
    return {
...
const AuthorPage = async ({ searchParams }: AuthorPageProps) => {
  const authorId = searchParams.id ? Number(searchParams.id) : 1;
  let data: AuthorDetailResponse | null = null;

  try {
    data = await fetchAuthorDetail(authorId);
  } catch (error) {
  ...
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <main style={{ backgroundColor: '#fdfdfd', flex: '1 0 auto' }}>
        ...
        <section className="author-hero pt-80 pb-60" style={{ 
                   ...
                   <div className="author-header d-flex flex-column flex-md-row align-items-center align-items-md-start gap-4 text-center text-md-start">
                      ...
                      <div className="author-info flex-grow-1 mt-2 mt-md-0">
                         <div className="d-flex flex-column flex-md-row justify-content-between align-items-center mb-2">
                             <div>
                                <h1 className="fw-bold mb-1" style={{ color: '#1a202c', fontSize: '2.5rem' }}>{author.name}</h1>
                                <p className="mb-0 fw-medium" style={{ color: '#007bff', fontSize: '1.1rem' }}>Devnot Yazarı</p>
                             </div>
                         </div>

                         <p className="mt-3 mb-4" style={{ fontSize: '1.05rem', lineHeight: '1.7', color: '#4a5568', maxWidth: '800px' }}>
                            {author.bio || "Teknoloji dünyasındaki gelişmeleri takip eden ve deneyimlerini Devnot okurlarıyla paylaşan değerli yazarımız."}
                         </p>

                         <div className="d-flex flex-column flex-sm-row align-items-center justify-content-between pt-3" style={{ borderTop: '1px solid #e2e8f0' }}>
                            <div className="d-flex gap-4 mb-3 mb-sm-0">
                                <div className="text-center text-sm-start">
                                    <span className="d-block fw-bold text-dark" style={{ fontSize: '1.2rem' }}>{articles.length}</span>
                                    <span className="d-block text-muted" style={{ fontSize: '0.85rem' }}>Makale</span>
                                </div>
                            </div>

                            <div className="social-links d-flex gap-3">
                                {author.linkedinUrl && (
                                    <Link href={author.linkedinUrl} target="_blank" className="text-secondary hover-primary" aria-label="LinkedIn">
                                        <i className="fab fa-linkedin-in fa-lg"></i>
                                    </Link>
                                )}
                                {author.twitterUrl && (
                                    <Link href={author.twitterUrl} target="_blank" className="text-secondary hover-primary" aria-label="Twitter">
                      ...
                      {articles.map((item, index) => {
                         ...
                         return (
                            <div key={index} className="col">
                               <article className="h-100 p-0 bg-white shadow-sm" style={{ 
